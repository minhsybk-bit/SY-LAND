...
export default function PdfToolkit() {
  ...
  function changeMode(next: Mode) { setMode(next); setFiles([]); setNotice(""); }
      ...
      <div className="pdf-tool-shell">
        ...
        <div className="pdf-tool-body">
          ...
          <div className="pdf-options">
            ...
            {mode !== "merge" && files[0] && <div className="single-pdf"><span>✓</span><p><b>{files[0].name}</b><small>{(files[0].size / 1024 / 1024).toFixed(1)} MB</small></p><button type="button" onClick={() => setFiles([])}>×</button></div>}
            <div className="pdf-run"><p className={notice.includes("Không") ? "error" : ""}>{notice || (files.length ? `${files.length} tệp · ${(totalSize / 1024 / 1024).toFixed(1)} MB` : "Kết quả sẽ được tải về thiết bị.")}</p><button type="button" disabled={busy || !files.length} onClick={() => void run()}>{busy ? "Đang xử lý…" : mode === "merge" ? "Nối PDF" : mode === "rotate" ? "Xoay và tải PDF" : "Tách và tải kết quả"}</button></div>
          </div>
        </div>
      </div>
    </section>
