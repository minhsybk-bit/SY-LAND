...
type Mode = "split" | "merge" | "rotate" | "organize" | "resize" | "annotate" | "optimize" | "compare" | "sanitize" | "ocr" | "compress" | "dedupe" | "toimage" | "imagetopdf" | "crop";
...
export default function PdfToolkit() {
  ...
  async function chooseFiles(event: ChangeEvent<HTMLInputElement>) {
    const selected = Array.from(event.target.files || []).filter((file) => mode === "imagetopdf" ? /image\/(jpeg|png)/i.test(file.type) || /\.(jpe?g|png)$/i.test(file.name) : file.type === "application/pdf" || file.name.toLowerCase().endsWith(".pdf"));
    setDuplicateGroups([]); setDuplicateScanned(false); setCompareResult(null);
    setFiles(mode === "merge" ? selected.slice(0, 30) : mode === "imagetopdf" ? selected.slice(0, 50) : mode === "compare" ? selected.slice(0, 2) : selected.slice(0, 1));
    ...
    try {
      ...
      if (mode === "imagetopdf") {
        ...
        for (let index = 0; index < files.length; index++) {
          ...
          const file = files[index]; const bytes = await file.arrayBuffer();
          const image = /png$/i.test(file.name) || file.type === "image/png" ? await output.embedPng(bytes) : await output.embedJpg(bytes);
  ...
  return (
    <section className="pdf-toolkit" id="cong-cu-pdf" aria-labelledby="pdf-toolkit-title">
      ...
      <div className="pdf-tool-shell">
        <div className="pdf-tool-finder"><label><span aria-hidden="true">⌕</span><input value={toolSearch} onChange={(event) => setToolSearch(event.target.value)} placeholder="Tìm công cụ: tách, OCR, nén, ảnh…" /></label><div>{([{ key: "all", label: "Tất cả" }, { key: "split", label: "Tách" }, { key: "edit", label: "Chỉnh sửa" }, { key: "convert", label: "Chuyển đổi" }, { key: "review", label: "Kiểm tra" }, { key: "optimize", label: "Tối ưu" }] as const).map((item) => <button className={toolCategory === item.key ? "active" : ""} type="button" key={item.key} onClick={() => setToolCategory(item.key)}>{item.label}</button>)}</div></div>
        <div className="pdf-tool-tabs" role="tablist" aria-label="Chọn công cụ PDF">
          ...
          {!(["split", "edit", "convert", "review", "optimize"] as const).some((category) => toolVisible(category, category === "split" ? "tách xuất trang chia đều chẵn lẻ khoảng mốc" : category === "edit" ? "nối ghép gộp xoay sắp xếp xóa cắt lề crop số trang dấu bản quyền watermark" : category === "convert" ? "chuyển khổ a3 a4 pdf sang ảnh jpg png ảnh sang pdf ghép ảnh chụp" : category === "review" ? "so sánh đối chiếu ocr scan trang trùng" : "tối ưu cấu trúc metadata nén dung lượng")) && <p className="tool-not-found">Không tìm thấy công cụ phù hợp.</p>}
        </div>
        <div className="pdf-tool-body">
          <div className="pdf-pick">
            <input ref={inputRef} type="file" accept={mode === "imagetopdf" ? "image/jpeg,image/png,.jpg,.jpeg,.png" : "application/pdf,.pdf"} multiple={mode === "merge" || mode === "compare" || mode === "imagetopdf"} onChange={chooseFiles} />
            <span aria-hidden="true">{mode === "imagetopdf" ? "IMG" : "PDF"}</span><h3>{mode === "merge" ? "Chọn nhiều PDF theo thứ tự cần nối" : mode === "imagetopdf" ? "Chọn ảnh JPG/PNG theo thứ tự" : mode === "compare" ? "Chọn hai phiên bản PDF" : "Chọn một tệp PDF"}</h3><p>{mode === "merge" ? "Tối đa 30 tệp. Có thể di chuyển thứ tự sau khi chọn." : mode === "imagetopdf" ? "Tối đa 50 ảnh. Có thể đổi thứ tự trước khi tạo PDF." : mode === "compare" ? "So sánh nội dung văn bản theo từng trang; không tải tệp lên máy chủ." : "Xử lý cục bộ; khuyến nghị tệp không quá 100 MB."}</p>
            <button type="button" onClick={() => inputRef.current?.click()}>{mode === "imagetopdf" ? "Chọn ảnh từ thiết bị" : "Chọn PDF từ thiết bị"}</button>
          </div>
