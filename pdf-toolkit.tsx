...
export default function PdfToolkit() {
  ...
  async function chooseFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    const countLimited = mode === "merge" || mode === "imagetopdf" || mode === "wordtopdf" || mode === "batch"
      ? selected.slice(0, MAX_BATCH_FILES)
    ...
    setNotice(!selected.length
      ...
      : accepted.length < selected.length
        ? `Đã nhận ${accepted.length}/${selected.length} tệp theo giới hạn an toàn: tối đa ${MAX_BATCH_FILES} tệp và ${Math.round(totalLimit / 1024 / 1024)} MB mỗi lượt.`
        : "");
    event.target.value = "";
    ...
    <section className="pdf-toolkit" id="cong-cu-pdf" aria-labelledby="pdf-toolkit-title">
      ...
      <div className="pdf-tool-shell">
        ...
        <div className="pdf-tool-body">
          <div className="pdf-workspace-summary"><div><span>KHÔNG GIAN XỬ LÝ</span><strong>{toolName}</strong></div><p><b>Tối đa 200 tệp/lượt</b><small>Xử lý tuần tự để giảm tải bộ nhớ · tệp luôn ở trên thiết bị</small></p></div>
          <div className="pdf-pick">
            ...
            <span aria-hidden="true">{mode === "imagetopdf" ? "IMG" : mode === "wordtopdf" ? "DOCX" : "PDF"}</span><h3>{mode === "merge" ? "Chọn nhiều PDF theo thứ tự cần nối" : mode === "batch" ? "Chọn nhiều PDF để xử lý hàng loạt" : mode === "wordtopdf" ? "Chọn nhiều tệp Word DOCX" : mode === "imagetopdf" ? "Chọn ảnh JPG/PNG/WebP theo thứ tự" : mode === "compare" ? "Chọn hai phiên bản PDF" : "Chọn một tệp PDF"}</h3><p>{mode === "merge" ? "Tối đa 200 tệp · 300 MB/lượt. Có thể di chuyển thứ tự sau khi chọn." : mode === "batch" ? "Tối đa 200 PDF · 300 MB/lượt. Kết quả và nhật ký CSV được đóng gói ZIP." : mode === "wordtopdf" ? "Tối đa 200 DOCX · 300 MB/lượt. Chuyển cục bộ, không tải tài liệu lên máy chủ." : mode === "imagetopdf" ? "Tối đa 200 ảnh · 500 MB/lượt. Có thể đổi thứ tự trước khi tạo PDF." : mode === "compare" ? "So sánh nội dung văn bản theo từng trang; không tải tệp lên máy chủ." : "Xử lý cục bộ; khuyến nghị tệp không quá 100 MB."}</p>
