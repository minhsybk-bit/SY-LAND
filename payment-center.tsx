...
export default function PaymentCenter() {
  ...
  async function useLicense(code: string) {
    try { await navigator.clipboard.writeText(code); } catch { /* Vẫn tiếp tục mở phần tài khoản. */ }
    window.dispatchEvent(new Event("syland-payment-updated"));
    document.getElementById("tai-khoan")?.scrollIntoView({ behavior: "smooth" });
    setMessage("Đã sao chép mã. Mở Trung tâm tài khoản để kích hoạt trên web hoặc phần mềm SỸ LAND.");
  }
