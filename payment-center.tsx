...
export default function PaymentCenter() {
  ...
  async function savePaymentConfig() {
    ...
    setBusy(true);
    try {
      const payload = { id: "primary", bank_bin: configDraft.bankBin.trim(), bank_name: configDraft.bankName.trim(), account_number: configDraft.accountNumber.trim(), account_name: configDraft.accountName.trim().toUpperCase(), support_phone: configDraft.supportPhone.trim() };
      await rest("/payment_settings?on_conflict=id", auth.accessToken, { method: "POST", payload, prefer: "resolution=merge-duplicates,return=representation" });
      setPaymentConfig({ bankBin: payload.bank_bin, bankName: payload.bank_name, accountNumber: payload.account_number, accountName: payload.account_name, supportPhone: payload.support_phone });
      setMessage("Đã lưu cấu hình nhận tiền an toàn trong Supabase. Thông tin không nằm trong mã nguồn GitHub.");
    } catch (reason) { setMessage(reason instanceof Error ? `${reason.message} Hãy chạy SUPABASE_PAYMENTS.sql phiên bản mới.` : "Không lưu được cấu hình nhận tiền."); }
