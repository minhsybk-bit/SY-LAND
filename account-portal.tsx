    ...
    <section className="account-portal" id="tai-khoan" aria-labelledby="account-title">
      ...
      {current.role === "admin" ? <div className="admin-grid">
        ...
        <div className="bug-workspace">
          ...
          <div className="admin-stats"><div><strong>{accounts.length}</strong><span>Tài khoản đã tải</span></div><div><strong>{licenses.filter((item) => item.status === "Hoạt động" && new Date(item.expiresAt) >= new Date()).length}</strong><span>Bản quyền hoạt động</span></div><div><strong>{devices.filter((item) => item.status === "Hoạt động").length}</strong><span>Thiết bị hoạt động</span></div><div><strong>{REMOTE_AUTH ? auditEvents.length : reports.filter((item) => item.status === "Mới").length}</strong><span>{REMOTE_AUTH ? "Sự kiện gần đây" : "Lỗi mới"}</span></div></div>
