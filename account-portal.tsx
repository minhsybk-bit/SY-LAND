    ...
    <section className="account-portal" id="tai-khoan" aria-labelledby="account-title">
      ...
      {current.role === "admin" ? <div className="admin-grid">
        ...
        <div className="bug-workspace">
          {REMOTE_AUTH && <section className="license-workspace"><div className="license-heading"><div><h3>Thông báo thanh toán</h3><p>Đơn người dùng đã báo chuyển khoản và đang chờ quản trị viên đối soát.</p></div><span>{paymentNotices.filter((item) => item.status === "Chờ xác nhận").length} CHỜ XÁC NHẬN</span></div><div className="license-list">{!paymentNotices.length ? <p>Chưa có đơn thanh toán.</p> : paymentNotices.slice(0, 20).map((item) => <article key={item.id}><div><strong>{item.orderCode} · Gói {item.plan}</strong>{item.licenseCode && <code>{item.licenseCode}</code>}<small>{item.status} · {new Date(item.createdAt).toLocaleString("vi-VN")}</small></div><span className={item.status === "Đã thanh toán" ? "active" : "locked"}>{item.status}</span></article>)}</div><a className="button button-primary" href="#thanh-toan">Mở đối soát và xác nhận tiền vào</a></section>}
