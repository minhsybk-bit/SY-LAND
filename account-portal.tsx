...
type License = { id: string; code: string; customer: string; email: string; plan: SylandPlan; expiresAt: string; status: "Hoạt động" | "Đã khóa"; createdAt: string; maxDevices?: number; seatCount?: number; maxParcelsPerRun?: number | null };
...
async function remoteAuth(path: string, payload: Record<string, unknown> = {}, options: { method?: "GET" | "POST" | "PUT"; accessToken?: string } = {}) {
  ...
  const response = await fetch(`${SUPABASE_URL}/auth/v1${path}`, {
    ...
    ...((options.method || "POST") === "GET" ? {} : { body: JSON.stringify(payload) }),
  });
...
export default function AccountPortal() {
  ...
  const activeLicense = useMemo(() => {
    ...
    if (REMOTE_AUTH) return licenses.filter((item) => item.email.trim().toLocaleLowerCase("vi-VN") === current.email.trim().toLocaleLowerCase("vi-VN") && item.status === "Hoạt động" && new Date(item.expiresAt) >= new Date()).sort((a, b) => new Date(b.expiresAt).getTime() - new Date(a.expiresAt).getTime())[0] || null;
  ...
  function openUpgrade(plan: "plus" | "pro") {
    window.dispatchEvent(new CustomEvent("syland-select-plan", { detail: { plan } }));
  ...
  useEffect(() => {
    async function restoreSession() {
      ...
      if (REMOTE_AUTH) {
        ...
        if (recoveryToken && hash.get("refresh_token")) {
          try {
            const userData = await remoteAuth("/user", {}, { method: "GET", accessToken: recoveryToken });
            const account = await hydrateRemoteAccount(userData, recoveryToken);
            ...
            history.replaceState(null, "", location.pathname + location.search);
            setMessage(reason instanceof Error ? reason.message : "Không hoàn tất được đăng nhập Google.");
          }
        }
  ...
  async function sendMagicLink() {
    ...
    if (!normalizedEmail || !/^\S+@\S+\.\S+$/.test(normalizedEmail)) {
      setMessage("Hãy nhập email hợp lệ để nhận liên kết đăng nhập.");
      return;
    }
    try {
      ...
      await remoteAuth(`/otp?redirect_to=${redirectTo}`, {
        ...
        create_user: true,
        data: { full_name: name.trim(), terms_version: TERMS_VERSION, terms_accepted_at: new Date().toISOString() },
      });
      setMessage("Đã gửi liên kết đăng nhập một lần. Hãy kiểm tra email và cả thư rác.");
    } catch (reason) {
      setMessage(reason instanceof Error ? reason.message : "Không gửi được liên kết đăng nhập.");
    }
  }
  ...
  async function createLicense(event: FormEvent) {
    ...
    const item: License = { id: crypto.randomUUID(), code: generateLicenseCode(), customer: licenseDraft.customer.trim(), email: licenseDraft.email.trim().toLocaleLowerCase(), plan: licenseDraft.plan, expiresAt: expires.toISOString(), status: "Hoạt động", createdAt: new Date().toISOString(), maxDevices: Math.max(1, Math.min(100, licenseDraft.maxDevices)), seatCount: Math.max(1, Math.min(500, licenseDraft.seatCount)), maxParcelsPerRun: licenseDraft.maxParcelsPerRun > 0 ? licenseDraft.maxParcelsPerRun : null };
    if (REMOTE_AUTH) {
      if (!remoteToken || current?.role !== "admin") { setMessage("Chỉ quản trị viên máy chủ được cấp bản quyền."); return; }
      try {
        const rows = await remoteData("/licenses", remoteToken, { method: "POST", payload: { code: item.code, customer: item.customer, email: item.email, plan: item.plan, expires_at: item.expiresAt, status: item.status, max_devices: item.maxDevices, seat_count: item.seatCount, max_parcels_per_run: item.maxParcelsPerRun } });
        const saved = Array.isArray(rows) ? rows[0] : null;
        const remoteItem = saved ? { ...item, id: saved.id, createdAt: saved.created_at || item.createdAt } : item;
        setLicenses((values) => [remoteItem, ...values]); setLicenseDraft({ customer: "", email: "", plan: "Go", months: 12, maxDevices: 1, seatCount: 1, maxParcelsPerRun: 80 }); setMessage("Đã cấp bản quyền trên máy chủ."); return;
    ...
    setLicenseDraft({ customer: "", email: "", plan: "Go", months: 12, maxDevices: 1, seatCount: 1, maxParcelsPerRun: 80 }); setMessage("Đã tạo mã bản quyền kiểm thử.");
  }
    ...
    <section className="account-portal" aria-labelledby="account-title">
      ...
      {current.role === "admin" ? <div className="admin-grid">
        ...
        <div className="bug-workspace">
          ...
          <section className="license-workspace"><div className="license-heading"><div><h3>Quản lý bản quyền {REMOTE_AUTH ? "máy chủ" : "kiểm thử"}</h3><p>Cấp quyền theo gói, số tài khoản và hạn mức riêng của mã.</p></div><span>{REMOTE_AUTH ? "ĐỒNG BỘ" : "CỤC BỘ"}</span></div><form className="license-form" onSubmit={createLicense}><label>Khách hàng<input value={licenseDraft.customer} onChange={(event) => setLicenseDraft((value) => ({ ...value, customer: event.target.value }))} /></label><label>Email<input type="email" value={licenseDraft.email} onChange={(event) => setLicenseDraft((value) => ({ ...value, email: event.target.value }))} /></label><label>Gói<select value={licenseDraft.plan} onChange={(event) => setLicenseDraft((value) => ({ ...value, plan: event.target.value as License["plan"] }))}><option>Go</option><option>Plus</option><option>Pro</option><option>Văn phòng</option><option>Đơn vị</option></select></label><label>Số tài khoản<input type="number" min={1} max={500} value={licenseDraft.seatCount} onChange={(event) => setLicenseDraft((value) => ({ ...value, seatCount: Number(event.target.value) || 1 }))} /></label><label>Giới hạn thửa/lần<input type="number" min={0} max={100000} value={licenseDraft.maxParcelsPerRun} onChange={(event) => setLicenseDraft((value) => ({ ...value, maxParcelsPerRun: Math.max(0, Number(event.target.value) || 0) }))} /><small>0 = theo quyền gói</small></label><label>Số tháng<input type="number" min={1} max={36} value={licenseDraft.months} onChange={(event) => setLicenseDraft((value) => ({ ...value, months: Number(event.target.value) || 1 }))} /></label><label>Số thiết bị<input type="number" min={1} max={100} value={licenseDraft.maxDevices} onChange={(event) => setLicenseDraft((value) => ({ ...value, maxDevices: Number(event.target.value) || 1 }))} /></label><button type="submit">Tạo mã bản quyền</button></form><div className="license-list">{licenses.map((item) => <article key={item.id}><div><strong>{item.customer}</strong><code>{item.code}</code><small>{item.email} · {item.plan} · {item.seatCount || 1} tài khoản · {item.maxParcelsPerRun || "Theo gói"} thửa/lần</small></div><button type="button" className={item.status === "Hoạt động" ? "active" : "locked"} onClick={() => toggleLicense(item.id)}>{item.status}</button></article>)}</div></section>
  ...
  return (
    <section className="account-portal account-auth" aria-labelledby="account-title">
      ...
      <form className="auth-form" onSubmit={submitAccount}>
        ...
        {(mode === "register" || mode === "setup") && <label>Họ và tên<input value={name} onChange={(event) => setName(event.target.value)} autoComplete="name" /></label>}
        {mode !== "reset" && <label>Email<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" placeholder="Nhập email đã đăng ký" /></label>}
        {mode !== "forgot" && <label>Mật khẩu<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete={mode === "login" ? "current-password" : "new-password"} placeholder="Tối thiểu 8 ký tự" /></label>}
        {mode !== "login" && mode !== "forgot" && <label>Xác nhận mật khẩu<input type="password" value={confirm} onChange={(event) => setConfirm(event.target.value)} autoComplete="new-password" /></label>}
        {(mode === "register" || mode === "setup") && <label className="terms-consent"><input type="checkbox" checked={acceptedTerms} onChange={(event) => setAcceptedTerms(event.target.checked)} /><span>Đồng ý <a href="#phap-ly">Chính sách bảo mật và Điều khoản sử dụng</a> phiên bản {TERMS_VERSION}.</span></label>}
        <button type="submit" disabled={AUTH_CONFIG_ERROR}>{mode === "setup" ? "Tạo tài khoản admin" : mode === "register" ? "Tạo tài khoản" : mode === "forgot" ? "Gửi email khôi phục" : mode === "reset" ? "Lưu mật khẩu mới" : "Đăng nhập"}</button>
        {REMOTE_AUTH && mode === "login" && <button className="auth-secondary magic-link-button" type="button" onClick={() => void sendMagicLink()}>Gửi liên kết đăng nhập qua email</button>}
        {message && <p className="account-message">{message}</p>}
        {mode === "login" && REMOTE_AUTH && <button className="auth-secondary" type="button" onClick={() => { setMode("forgot"); setMessage(""); }}>Quên mật khẩu?</button>}
        {mode !== "setup" && mode !== "reset" && <p className="auth-switch">{mode === "login" ? <>Chưa có tài khoản? <button type="button" onClick={() => { setMode("register"); setMessage(""); }}>Đăng ký tài khoản</button></> : <>Đã có tài khoản? <button type="button" onClick={() => { setMode("login"); setMessage(""); }}>Đăng nhập</button></>}</p>}
        {REMOTE_AUTH && mode !== "forgot" && mode !== "reset" && <small className="social-auth-consent">Khi tiếp tục, người dùng đồng ý Chính sách bảo mật và Điều khoản sử dụng SỸ LAND. Mỗi tài khoản có vùng dữ liệu độc lập.</small>}
      </form>
    </section>
  );
}
